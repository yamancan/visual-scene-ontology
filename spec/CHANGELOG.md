...
*Annotation, 2026-08-01 — the grammars become executable.* Appendix B and
Appendix D have been the normative grammars for the two syntaxes a person writes
by hand since v1.0 and v1.1 respectively, and through v1.3.0 nothing ran them.
§D.9 records the one hand reconciliation they ever had. `make grammar-check`
(new, inside `make check`) extracts the `ebnf` blocks of Appendix B and of §D.2,
§D.3 and §D.5 out of `docs/vson.md` by heading, translates them into an LALR(1)
grammar under twelve numbered rules, and runs the result over the corpus: no
module under `tools/grammar/` carries a copy of a production, a terminal or a
token vocabulary, so a grammar that changes in the spec changes there in the
same commit or the gate goes red. New §D.10 states all of it.

**What the gate asserts.** The generated parsers accept `throne_room` and all
sixteen gallery scenes on the VSON-P side and all twelve gallery-x scenes on the
VSON-X side, and so do the two reference implementations. §D.7's rows now carry
an identifier and a **Decided by** column, and `tests/fixtures/grammar/negative/`
holds one document per row: the reference parser must reject all eighteen, the
generated parser must reject exactly the ten the column calls `grammar`. §D.3's
five closed vocabularies are stated as productions for the first time and pinned
four ways — against the counts the prose spells out, against the reference
lexer's constants, against `rcc_values` in `routing-tables.json`, and against a
fixture per vocabulary that must be rejected.

**What it found, and what moved.** Five mismatches between the appendices and
the parser that has shipped all along, all resolved the way §D.9 resolved its
six — the parser is authoritative — and all tabled in §D.10: Appendix B's `role`
made `: agent` legal, its `unit` stopped at the first non-letter, its `term` was
ambiguous between `var` and `bareword` and so could not be compiled at all, both
appendices let a string escape span a line, and §D.5's `arglist` admitted a
`~MOD` on a thematic role that the parser rejects unconditionally. The last one
is the only change to the accepted language of VSON-X, and it narrows the
*grammar* to what the parser already did: `arg_kv` is `kv` without the tail,
which moves §D.7 E10 from `parser` to `grammar`. No document this repository
ships is affected, no clause, shape, term, schema field or IRI moves, and §D.9's
six rows are annotated rather than rewritten.

**Constrained decoding.** `tools/grammar/vson-x.gbnf` is a generated llama.cpp
GBNF translation of Appendix D, regenerated by `make grammar-gbnf` and compared
byte for byte by the gate. GBNF is the target because it is an open format with
a public parser, tied to no vendor and no model. §D.10 states what it does not
guarantee, in the plainest terms the project can manage: it constrains
characters and nothing else, a document sampled under it is syntactically
VSON-X and not conformant VSON-X, it is deliberately wider than the appendix
wherever GBNF cannot express a lookahead, and it is checked here against its own
re-read and the shipped corpus — not against llama.cpp, which this repository
does not run. `lark` joins `pyproject.toml` as a `dev` extra, never a runtime
