...
- **`vson validate --format compact`** (new §5.16.8). One line per finding —
  ...
  the same `OK` / `FAIL` line `--format text` prints, per input. It answers the
  two readers nobody writes a parser for: a person scrolling a failed build,
  and a `grep`. A message written across several lines in the shapes file is
  collapsed to one space per whitespace run, so a finding is a line and
  `grep -c` counts findings; a finding whose position §5.16.3 never established
  prints its path alone rather than a plausible `:1:1`. It is a rendering
  rather than a document format: it carries no version, drops every field
  beyond those three instead of abbreviating them, and §5.16.6 still points a
  repair loop at `--format json`. The verdict, the exit codes and the gate
  attribution are unchanged — the format decides nothing.
- **A release workflow** (`.github/workflows/release.yml`). A `v*` tag builds
  the CLI for macOS (arm64 and x86_64), Linux (x86_64, musl, statically
  linked) and Windows (x86_64), names each binary `vson-<version>-<platform>`,
  verifies the checksum it computed on the build runner after the bytes have
  crossed the artifact store, and attaches the four assets plus a `SHA256SUMS`
  file to the release. The build jobs read; only the job that attaches assets
  is granted `contents: write`. Two things it refuses to do: publish a binary
  whose `--version` does not match the tag that asked for it, and publish a
  release that is missing a platform. Hand-rolled rather than generated by
  cargo-dist — the workflow's own header records why, and what that choice
  costs. This is plumbing for the tag, not a claim that binaries exist: the
  first release it builds is the one that mints them.
- **Issue and pull-request templates.** A bug report form that asks for the
  failing document and the verifier's own output, a specification-question form
  that asks which clause is being read, a `config.yml` that routes an unfixed
